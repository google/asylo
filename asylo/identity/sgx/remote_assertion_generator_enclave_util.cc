...
StatusOr<SealedSecret> CreateSealedSecret(
  ...
  ASYLO_ASSIGN_OR_RETURN(*enclave_secret.mutable_attestation_key(),
...
StatusOr<AsymmetricSigningKeyProto> GetAsymmetricSigningKeyProto(
  ...
  ASYLO_RETURN_IF_ERROR(attestation_key.SerializeToDer(&serialized_key));
  asymmetric_signing_key_proto.set_key(CopyToByteContainer<std::string>(
      {serialized_key.data(), serialized_key.size()}));
  asymmetric_signing_key_proto.set_encoding(
      AsymmetricKeyEncoding::ASYMMETRIC_KEY_DER);
  asymmetric_signing_key_proto.set_key_type(
      AsymmetricSigningKeyProto::SIGNING_KEY);
  asymmetric_signing_key_proto.set_signature_scheme(
      attestation_key.GetSignatureScheme());
