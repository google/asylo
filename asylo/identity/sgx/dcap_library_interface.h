...
#ifndef ASYLO_IDENTITY_SGX_DCAP_LIBRARY_INTERFACE_H_
#define ASYLO_IDENTITY_SGX_DCAP_LIBRARY_INTERFACE_H_
...
class DcapLibraryInterface {
 public:
  ...
  virtual quote3_error_t qe_set_enclave_dirpath(const char *dirpath) const = 0;

  // Wraps sgx_pce_get_target. Returns a value from the `sgx_pce_error_t`
  // enumeration to indicate status.
  virtual sgx_pce_error_t pce_get_target(
      sgx_target_info_t *p_pce_target, sgx_isv_svn_t *p_pce_isv_svn) const = 0;
...
#endif  // ASYLO_IDENTITY_SGX_DCAP_LIBRARY_INTERFACE_H_
